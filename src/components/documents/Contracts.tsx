          ...
          <div className="bg-white rounded-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-6 space-y-5">
              ...
              <div>
                ...
                <div className="space-y-2">{items.map((item, idx) => <div key={idx} className="flex flex-col sm:flex-row gap-2 items-start sm:items-center p-2 bg-slate-50 rounded-lg"><select value={item.product_id || ''} onChange={e => selectProduct(idx, e.target.value)} className="w-full sm:w-40 px-2 py-1.5 border border-slate-200 rounded text-xs bg-white"><option value="">Select product</option>{products.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select><input value={item.description || ''} onChange={e => updateItem(idx, 'description', e.target.value)} placeholder="Description" className="flex-1 w-full px-2 py-1.5 border border-slate-200 rounded text-xs" /><input type="number" value={item.quantity || ''} onChange={e => updateItem(idx, 'quantity', parseInt(e.target.value) || 0)} placeholder="Qty" className="w-20 px-2 py-1.5 border border-slate-200 rounded text-xs" /><input type="number" step="0.01" value={item.unit_price || ''} onChange={e => updateItem(idx, 'unit_price', parseFloat(e.target.value) || 0)} placeholder="Unit Price" className="w-24 px-2 py-1.5 border border-slate-200 rounded text-xs" /><span className="text-xs font-medium text-slate-600 w-24 text-right">{formatCurrency((item.quantity || 0) * (item.unit_price || 0), form.currency || 'USD')}</span><button onClick={() => removeItem(idx)} className="p-1 text-slate-400 hover:text-red-500"><X className="w-4 h-4" /></button></div>)}</div>
              </div>
          ...
          <div className="grid grid-cols-2 gap-6 mb-6 text-sm">
            ...
            <div>
              ...
              <div className="space-y-1">
                ...
                {contract.destination_country && <div className="flex justify-between"><span className="text-slate-400">Destination:</span><span className="text-slate-700">{contract.destination_country}</span></div>}
                {contract.origin_country && <div className="flex justify-between"><span className="text-slate-400">Origin:</span><span className="text-slate-700">{contract.origin_country}</span></div>}
                {contract.port_of_loading && <div className="flex justify-between"><span className="text-slate-400">Loading Port:</span><span className="text-slate-700">{contract.port_of_loading}</span></div>}
                {contract.port_of_discharge && <div className="flex justify-between"><span className="text-slate-400">Discharge Port:</span><span className="text-slate-700">{contract.port_of_discharge}</span></div>}
