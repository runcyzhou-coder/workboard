      ...
      <div className="bg-white rounded-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto print:max-h-none print:rounded-none print:shadow-none" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-8 print:p-10" id="pi-document">
          ...
          <div className="grid grid-cols-2 gap-6 mb-6">
            <div className="bg-slate-50 rounded-lg p-4"><p className="text-xs font-semibold text-slate-400 uppercase mb-2">Buyer</p><p className="text-sm font-medium text-slate-900">{contract.customer?.company_name || '—'}</p>{contract.customer?.address && <p className="text-sm text-slate-600">{contract.customer.address}</p>}<p className="text-sm text-slate-600">{contract.customer?.country || ''}</p>{contract.customer?.email && <p className="text-sm text-slate-600">{contract.customer.email}</p>}</div>
          ...
          {(contract.inspection_clause || contract.warranty_clause || contract.force_majeure || contract.arbitration_clause) && (
            <div className="space-y-3 mb-6"><p className="text-xs font-semibold text-slate-400 uppercase">Terms & Conditions</p>{contract.inspection_clause && <div className="bg-slate-50 p-3 rounded-lg"><p className="text-xs font-medium text-slate-500 mb-1">检验条款 Inspection</p><p className="text-sm text-slate-600">{contract.inspection_clause}</p></div>}{contract.warranty_clause && <div className="bg-slate-50 p-3 rounded-lg"><p className="text-xs font-medium text-slate-500 mb-1">质量保证 Warranty</p><p className="text-sm text-slate-600">{contract.warranty_clause}</p></div>}{contract.force_majeure && <div className="bg-slate-50 p-3 rounded-lg"><p className="text-xs font-medium text-slate-500 mb-1">不可抗力 Force Majeure</p><p className="text-sm text-slate-600">{contract.force_majeure}</p></div>}{contract.arbitration_clause && <div className="bg-slate-50 p-3 rounded-lg"><p className="text-xs font-medium text-slate-500 mb-1">仲裁条款 Arbitration</p><p className="text-sm text-slate-600">{contract.arbitration_clause}</p></div>}</div>
          )}
