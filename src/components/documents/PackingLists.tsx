          ...
          <div className="bg-white rounded-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-6 space-y-5">
              ...
              <div>
                ...
                <div className="space-y-2">{items.map((item, idx) => <div key={idx} className="p-2 bg-slate-50 rounded-lg space-y-2"><div className="flex gap-2 items-center"><select value={item.product_id || ''} onChange={e => selectProduct(idx, e.target.value)} className="w-full sm:w-32 px-2 py-1.5 border border-slate-200 rounded text-xs bg-white"><option value="">Select product</option>{products.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select><input value={item.description || ''} onChange={e => updateItem(idx, 'description', e.target.value)} placeholder="Product Description" className="flex-1 px-2 py-1.5 border border-slate-200 rounded text-xs" /><button onClick={() => removeItem(idx)} className="p-1 text-slate-400 hover:text-red-500"><X className="w-4 h-4" /></button></div><div className="grid grid-cols-2 sm:grid-cols-7 gap-2"><input type="number" value={item.quantity || ''} onChange={e => updateItem(idx, 'quantity', parseInt(e.target.value) || 0)} placeholder="Qty" className="px-2 py-1.5 border border-slate-200 rounded text-xs" /><input type="number" value={item.package_count || ''} onChange={e => updateItem(idx, 'package_count', parseInt(e.target.value) || 0)} placeholder="Packages" className="px-2 py-1.5 border border-slate-200 rounded text-xs" /><input value={item.package_type || ''} onChange={e => updateItem(idx, 'package_type', e.target.value)} placeholder="Package" className="px-2 py-1.5 border border-slate-200 rounded text-xs" /><input type="number" step="0.001" value={item.gross_weight || ''} onChange={e => updateItem(idx, 'gross_weight', parseFloat(e.target.value) || 0)} placeholder="Gross kg" className="px-2 py-1.5 border border-slate-200 rounded text-xs" /><input type="number" step="0.001" value={item.net_weight || ''} onChange={e => updateItem(idx, 'net_weight', parseFloat(e.target.value) || 0)} placeholder="Net kg" className="px-2 py-1.5 border border-slate-200 rounded text-xs" /><input type="number" step="0.1" value={item.length ?? ''} onChange={e => updateItem(idx, 'length', parseFloat(e.target.value) || 0)} placeholder="L cm" className="px-2 py-1.5 border border-slate-200 rounded text-xs" /><input type="number" step="0.1" value={item.width ?? ''} onChange={e => updateItem(idx, 'width', parseFloat(e.target.value) || 0)} placeholder="W cm" className="px-2 py-1.5 border border-slate-200 rounded text-xs" /></div><div className="flex items-center gap-2"><input type="number" step="0.1" value={item.height ?? ''} onChange={e => updateItem(idx, 'height', parseFloat(e.target.value) || 0)} placeholder="H cm" className="w-24 px-2 py-1.5 border border-slate-200 rounded text-xs" /><span className="text-xs text-slate-500">Volume: {calcVol(item.length ?? null, item.width ?? null, item.height ?? null).toFixed(3)} m³</span></div></div>)}</div>
              </div>
          ...
          <div className="grid grid-cols-2 gap-6 mb-6 text-sm">
            <div>
              ...
              <div className="space-y-1">
                <div className="flex"><span className="w-24 shrink-0 text-slate-400">Cont.Pers:</span><span className="text-slate-700">{pl.customer?.contact_name || '—'}</span></div>
                <div className="flex"><span className="w-24 shrink-0 text-slate-400">Company:</span><span className="text-slate-700">{pl.customer?.company_name || '—'}</span></div>
                <div className="flex"><span className="w-24 shrink-0 text-slate-400">Address:</span><span className="text-slate-700">{pl.customer?.address || '—'}</span></div>
                <div className="flex"><span className="w-24 shrink-0 text-slate-400">Tel. No.:</span><span className="text-slate-700">{pl.customer?.phone || '—'}</span></div>
                <div className="flex"><span className="w-24 shrink-0 text-slate-400">E-mail:</span><span className="text-slate-700">{pl.customer?.email || '—'}</span></div>
              </div>
            </div>
            <div>
              ...
              <div className="space-y-1">
                ...
                {pl.shipping_method && <div className="flex justify-between"><span className="text-slate-400">Shipping:</span><span className="text-slate-700">{pl.shipping_method}</span></div>}
                {pl.vessel_name && <div className="flex justify-between"><span className="text-slate-400">Vessel:</span><span className="text-slate-700">{pl.vessel_name}</span></div>}
                {pl.bl_number && <div className="flex justify-between"><span className="text-slate-400">B/L No.:</span><span className="text-slate-700">{pl.bl_number}</span></div>}
                {pl.container_number && <div className="flex justify-between"><span className="text-slate-400">Container No.:</span><span className="text-slate-700">{pl.container_number}</span></div>}
                {pl.port_of_loading && <div className="flex justify-between"><span className="text-slate-400">Loading Port:</span><span className="text-slate-700">{pl.port_of_loading}</span></div>}
                {pl.port_of_discharge && <div className="flex justify-between"><span className="text-slate-400">Discharge Port:</span><span className="text-slate-700">{pl.port_of_discharge}</span></div>}
              </div>
            </div>
          </div>
