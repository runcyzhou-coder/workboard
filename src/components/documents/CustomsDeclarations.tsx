          ...
          <div className="bg-white rounded-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-6 space-y-5">
              ...
              <div>
                ...
                <div className="space-y-2">{items.map((item, idx) => <div key={idx} className="p-2 bg-slate-50 rounded-lg space-y-2"><div className="flex gap-2 items-center"><select value={item.product_id || ''} onChange={e => selectProduct(idx, e.target.value)} className="w-full sm:w-32 px-2 py-1.5 border border-slate-200 rounded text-xs bg-white"><option value="">Select product</option>{products.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select><input value={item.description || ''} onChange={e => updateItem(idx, 'description', e.target.value)} placeholder="Product Description" className="flex-1 px-2 py-1.5 border border-slate-200 rounded text-xs" /><button onClick={() => removeItem(idx)} className="p-1 text-slate-400 hover:text-red-500"><X className="w-4 h-4" /></button></div><div className="grid grid-cols-2 sm:grid-cols-6 gap-2"><input value={item.hs_code || ''} onChange={e => updateItem(idx, 'hs_code', e.target.value)} placeholder="HS Code" className="px-2 py-1.5 border border-slate-200 rounded text-xs" /><input type="number" value={item.quantity || ''} onChange={e => updateItem(idx, 'quantity', parseInt(e.target.value) || 0)} placeholder="Qty" className="px-2 py-1.5 border border-slate-200 rounded text-xs" /><input value={item.unit || ''} onChange={e => updateItem(idx, 'unit', e.target.value)} placeholder="Unit" className="px-2 py-1.5 border border-slate-200 rounded text-xs" /><input type="number" step="0.01" value={item.unit_price || ''} onChange={e => updateItem(idx, 'unit_price', parseFloat(e.target.value) || 0)} placeholder="Unit Price" className="px-2 py-1.5 border border-slate-200 rounded text-xs" /><input type="number" step="0.001" value={item.gross_weight || ''} onChange={e => updateItem(idx, 'gross_weight', parseFloat(e.target.value) || 0)} placeholder="Gross kg" className="px-2 py-1.5 border border-slate-200 rounded text-xs" /><input type="number" step="0.001" value={item.net_weight || ''} onChange={e => updateItem(idx, 'net_weight', parseFloat(e.target.value) || 0)} placeholder="Net kg" className="px-2 py-1.5 border border-slate-200 rounded text-xs" /></div></div>)}</div>
              </div>
