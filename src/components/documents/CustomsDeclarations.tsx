      ...
      <div className="bg-white rounded-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto print:max-h-none print:rounded-none print:shadow-none" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-8 print:p-10" id="pi-document">
          ...
          <div className="grid grid-cols-2 gap-6 mb-6">
            <div className="bg-slate-50 rounded-lg p-4"><p className="text-xs font-semibold text-slate-400 uppercase mb-2">Consignee</p><p className="text-sm font-medium text-slate-900">{cd.customer?.company_name || '—'}</p>{cd.customer?.address && <p className="text-sm text-slate-600">{cd.customer.address}</p>}<p className="text-sm text-slate-600">{cd.customer?.country || ''}</p></div>
          ...
          <table className="w-full text-sm mb-6"><thead><tr className="bg-slate-900 text-white"><th className="px-3 py-2 text-left font-medium rounded-l">序号</th><th className="px-3 py-2 text-left font-medium">Description</th><th className="px-3 py-2 text-left font-medium">HS Code</th><th className="px-3 py-2 text-right font-medium">Qty</th><th className="px-3 py-2 text-left font-medium">Unit</th><th className="px-3 py-2 text-right font-medium">毛重</th><th className="px-3 py-2 text-right font-medium rounded-r">净重</th></tr></thead><tbody>{cd.items.length === 0 ? <tr><td colSpan={7} className="text-center py-6 text-slate-400">No items</td></tr> : cd.items.map((item, idx) => <tr key={item.id} className="border-b border-slate-100"><td className="px-3 py-2.5 text-slate-500">{idx + 1}</td><td className="px-3 py-2.5 text-slate-900">{item.description}</td><td className="px-3 py-2.5 text-slate-600">{item.hs_code || '—'}</td><td className="px-3 py-2.5 text-right text-slate-600">{item.quantity}</td><td className="px-3 py-2.5 text-slate-600">{item.unit}</td><td className="px-3 py-2.5 text-right text-slate-600">{item.gross_weight} kg</td><td className="px-3 py-2.5 text-right text-slate-600">{item.net_weight} kg</td></tr>)}</tbody></table>
