          ...
          <div className="bg-white rounded-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-6 space-y-5">
              ...
              <div>
                ...
                <div className="space-y-2">{items.map((item, idx) => <div key={idx} className="flex flex-col sm:flex-row gap-2 items-start sm:items-center p-2 bg-slate-50 rounded-lg"><select value={item.product_id || ''} onChange={e => selectProduct(idx, e.target.value)} className="w-full sm:w-40 px-2 py-1.5 border border-slate-200 rounded text-xs bg-white"><option value="">选择产品</option>{products.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select><input value={item.description || ''} onChange={e => updateItem(idx, 'description', e.target.value)} placeholder="描述" className="flex-1 w-full px-2 py-1.5 border border-slate-200 rounded text-xs" /><input type="number" value={item.quantity || ''} onChange={e => updateItem(idx, 'quantity', parseInt(e.target.value) || 0)} placeholder="数量" className="w-20 px-2 py-1.5 border border-slate-200 rounded text-xs" /><input type="number" step="0.01" value={item.unit_price || ''} onChange={e => updateItem(idx, 'unit_price', parseFloat(e.target.value) || 0)} placeholder="单价" className="w-24 px-2 py-1.5 border border-slate-200 rounded text-xs" /><span className="text-xs font-medium text-slate-600 w-24 text-right">{formatCurrency((item.quantity || 0) * (item.unit_price || 0), form.currency || 'USD')}</span><button onClick={() => removeItem(idx)} className="p-1 text-slate-400 hover:text-red-500"><X className="w-4 h-4" /></button></div>)}</div>
              </div>
              <div className="grid grid-cols-3 gap-4 border-t border-slate-100 pt-4">
                <Field label="运费"><input type="number" step="0.01" value={form.freight || ''} onChange={e => setForm({ ...form, freight: parseFloat(e.target.value) || 0 })} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" /></Field>
                <Field label="保险"><input type="number" step="0.01" value={form.insurance || ''} onChange={e => setForm({ ...form, insurance: parseFloat(e.target.value) || 0 })} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" /></Field>
                <Field label="其他费用"><input type="number" step="0.01" value={form.other_charges || ''} onChange={e => setForm({ ...form, other_charges: parseFloat(e.target.value) || 0 })} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" /></Field>
              </div>
          ...
          <div className="grid grid-cols-2 gap-6 mb-6 text-sm">
            <div>
              ...
              <div className="space-y-1">
                <div className="flex"><span className="w-24 shrink-0 text-slate-400">Cont.Pers:</span><span className="text-slate-700">{ci.customer?.contact_name || '—'}</span></div>
                <div className="flex"><span className="w-24 shrink-0 text-slate-400">Company:</span><span className="text-slate-700">{ci.customer?.company_name || '—'}</span></div>
                <div className="flex"><span className="w-24 shrink-0 text-slate-400">Address:</span><span className="text-slate-700">{ci.customer?.address || '—'}</span></div>
                <div className="flex"><span className="w-24 shrink-0 text-slate-400">Tel. No.:</span><span className="text-slate-700">{ci.customer?.phone || '—'}</span></div>
                <div className="flex"><span className="w-24 shrink-0 text-slate-400">E-mail:</span><span className="text-slate-700">{ci.customer?.email || '—'}</span></div>
              </div>
            </div>
            <div>
              <p className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-2">COMMERCIAL INVOICE DETAILS</p>
              <div className="space-y-1">
                <div className="flex justify-between"><span className="text-slate-400">Date:</span><span className="text-slate-700">{formatDate(ci.created_at)}</span></div>
                <div className="flex justify-between"><span className="text-slate-400">Trade Terms:</span><span className="text-slate-700">{ci.delivery_terms || '—'}</span></div>
                ...
                {ci.shipping_method && <div className="flex justify-between"><span className="text-slate-400">Shipping:</span><span className="text-slate-700">{ci.shipping_method}</span></div>}
                {ci.origin_country && <div className="flex justify-between"><span className="text-slate-400">Origin:</span><span className="text-slate-700">{ci.origin_country}</span></div>}
                {ci.destination_country && <div className="flex justify-between"><span className="text-slate-400">Destination:</span><span className="text-slate-700">{ci.destination_country}</span></div>}
                {ci.port_of_loading && <div className="flex justify-between"><span className="text-slate-400">Loading Port:</span><span className="text-slate-700">{ci.port_of_loading}</span></div>}
                {ci.port_of_discharge && <div className="flex justify-between"><span className="text-slate-400">Discharge Port:</span><span className="text-slate-700">{ci.port_of_discharge}</span></div>}
                {ci.vessel_name && <div className="flex justify-between"><span className="text-slate-400">Vessel:</span><span className="text-slate-700">{ci.vessel_name}</span></div>}
                {ci.bl_number && <div className="flex justify-between"><span className="text-slate-400">B/L No.:</span><span className="text-slate-700">{ci.bl_number}</span></div>}
              </div>
            </div>
          </div>
          <div className="border-t border-slate-200 mb-4" />
          <table className="w-full text-sm mb-6"><thead><tr className="bg-slate-900 text-white"><th className="px-3 py-2 text-center font-medium rounded-l">#</th><th className="px-3 py-2 text-left font-medium">Description</th><th className="px-3 py-2 text-right font-medium">Qty</th><th className="px-3 py-2 text-right font-medium">Unit Price</th><th className="px-3 py-2 text-right font-medium rounded-r">Amount</th></tr></thead><tbody>{ci.items.length === 0 ? <tr><td colSpan={5} className="text-center py-6 text-slate-400">No items</td></tr> : ci.items.map((item, idx) => <tr key={item.id} className="border-b border-slate-100"><td className="px-3 py-2.5 text-center text-slate-500">{idx + 1}</td><td className="px-3 py-2.5 text-left text-slate-900">{item.description}</td><td className="px-3 py-2.5 text-right text-slate-600">{item.quantity}</td><td className="px-3 py-2.5 text-right text-slate-600">{formatCurrency(item.unit_price, ci.currency)}</td><td className="px-3 py-2.5 text-right font-medium text-slate-900">{formatCurrency(item.total, ci.currency)}</td></tr>)}</tbody></table>
